...
class LogStash::Filters::Ssdeep < LogStash::Filters::Base
  ...
  config :aerospike_set_fuzzy_hash,         :validate => :string,           :default => "ssdeep"
  #Set in Aerospike is similar to table in a relational database.
  ...
  def get_ssdeep_info
    ...
    #@ssdeep = Ssdeep.from_file(@file_path)
    _,_,_,@ssdeep,_ = `#{@ssdeep_bin} -s #{@file_path}`.split(/[,\n]/)

    ssdeep_records = get_ssdeep_records

    score = 0 if ssdeep_records.records.size == 0

    ssdeep_records.each do |record|
      key = record.key.user_key
      if @hash != key  #Avoid duplicate events
        record.bins.each do |_, ssdeep_hash|
  ...
  def update_aerospike(matches,score)

    begin
      #if !matches.empty? || @file_score > 0   #TODO Check score to know if we should add the fuzzy hash to aerospike
        # First add ssdeep hash to @aerospike_set_fuzzy_hash
        key = Key.new(@aerospike_namespace,@aerospike_set_fuzzy_hash,@hash)
        bin = Bin.new(@target, @ssdeep)
        @aerospike.put(key,bin,WritePolicy.new)
      #end
  ...
  def filter(event)
    ...
    ssdeep_result, score, matches = get_ssdeep_info
    update_aerospike(matches,score)
